...
def remove_duplicate_compare(instruction):
    
    for i in range(len(instruction)): #ciclo sugli attemp 
        count_duplicate=0
        for j in range(len(instruction[i])): #instruction  
            if(instruction[i][j][0]=='Ultrasonic Sensor Compare CM'):
                count_duplicate+=1
            if(count_duplicate>1):               
                new_attemp=np.delete(instruction[i],j, axis=0)
                new_attemp=np.append(new_attemp, [np.empty(6, dtype='U256')], axis=0)
                instruction[i]=new_attemp
    return instruction
...
for filename in glob.glob(input_path):
    if filename.endswith(".rtf"):
        ...
        #rimozione del campo condition, che rappresenta solo la condizione e non viene impostata dai ragazzi
        df1 = df1.replace(to_replace =["Condition = 0;", "Condition = 1;"], 
                            value =np.NaN, regex=True)
        
        #rimozione delle istruzioni dei cicli
        df1 = df1.drop(df1[(df1['Blockname'] == "Start Loop Cycle") | (df1['Blockname']=='EndLoopCycle')].index)
